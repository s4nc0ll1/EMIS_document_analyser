...
class SentimentAnalyzer:
    ...
    def analyze_text(self, text: str) -> SentimentAnalysis:
        if not text or not text.strip():
            return SentimentAnalysis(sentiment="Unknown", score=0.0, analysis_text="No text to analyze.")
        ...
        return SentimentAnalysis(sentiment=sentiment, score=compound_score, analysis_text=analysis_text)
...
class DocumentAnalyzer:
    ...
    def analyze_documents(self, documents: List[Dict[str, Any]]) -> pd.DataFrame:
        ...
        for doc in documents:
            ...
            analyses.append({"date": doc.get("date"), "sentiment": analysis_result.sentiment})
...
class SessionStateManager:
    ...
    SESSION_KEYS = ["logged_in", "result", "general_analysis_df", "search_info", "emis_api_token"]
    
    @staticmethod
    def initialize() -> None:
        defaults = {"logged_in": False, "result": None, "general_analysis_df": None, "search_info": None, "emis_api_token": None}
...
class UIComponents:
    ...
    SENTIMENT_COLORS = {"Positive": "green", "Negative": "red", "Neutral": "orange", "Unknown": "grey"}
    ...
    def render_search_filters() -> SearchParameters:
        ...
        selected_country = st.sidebar.selectbox("Country Name", options=country_list, index=0)
        company_input = st.sidebar.text_input("Company Name")
        selected_industry = st.sidebar.selectbox("Industry", options=industry_list, index=0)
        keyword_input = st.sidebar.text_input("Keyword")
        ...
        doc_limit = st.sidebar.number_input(
            ...
            value=200, 
            step=100,
            help="Total number of documents to retrieve. The API fetches them in pages of 100."
        )
    ...
    def render_search_info(search_info: Dict[str, Any]) -> None:
        ...
        with st.container(border=True):
            ...
            if search_info.get("country_name"):
                st.markdown(f"**Country:** `{search_info['country_name']}`   **→**   **Resolved Code:** `{search_info.get('country_code', 'Not Found')}`")
            if search_info.get("industry_name"):
                st.markdown(f"**Industry:** `{search_info['industry_name']}`   **→**   **Resolved Code:** `{search_info.get('industry_code', 'Not Found')}`")
    ...
    def _render_sentiment_distribution(cls, df: pd.DataFrame) -> None:
        ...
        fig_bar = px.bar(sentiment_counts, x=sentiment_counts.index, y=sentiment_counts.values,
                         labels={'x': 'Sentiment', 'y': 'Number of Documents'}, title="Total Count by Sentiment",
                         color=sentiment_counts.index, color_discrete_map=cls.SENTIMENT_COLORS)
        fig_bar.update_layout(showlegend=False)
        st.plotly_chart(fig_bar, use_container_width=True)
    
    @classmethod
    def _render_sentiment_timeline(cls, df: pd.DataFrame) -> None:
        time_series_data = df.groupby(['date', 'sentiment']).size().reset_index(name='count').sort_values('date')
        fig_line = px.line(time_series_data, x='date', y='count', color='sentiment',
                           labels={'date': 'Date', 'count': 'Number of Documents', 'sentiment': 'Sentiment'},
                           title="Daily Sentiment Trend", markers=True, color_discrete_map=cls.SENTIMENT_COLORS)
    ...
    @classmethod
    def render_topics_wordcloud(cls, documents: List[Dict[str, Any]]) -> None:
        """Render an aesthetically improved word cloud for topics."""
        st.subheader("Content Analysis: Topics Word Cloud")
        ...
        topic_freq = dict(zip(topic_df['name'], topic_df['count']))
        
        # Custom color palette - modern and professional
        colors = ['#2E86AB', '#A23B72', '#F18F01', '#C73E1D', '#6A994E', 
                '#7209B7', '#F72585', '#4361EE', '#F77F00', '#FCBF49']
        
        def color_func(word, font_size, position, orientation, random_state=None, **kwargs):
            """Custom color function for more control over colors."""
            import random
            random.seed(hash(word) % 2147483647)  # Consistent colors for same words
        ...
        # Display word cloud with better interpolation
...
class DocumentFilter:
    ...
    def extract_unique_dates(documents: List[Dict[str, Any]]) -> List[str]:
        ...
        for doc in documents:
            if doc_date := doc.get("date"):
                try: dates.add(str(pd.to_datetime(doc_date).date()))
                except Exception: dates.add(doc_date)
        return sorted(list(dates))
    
    @staticmethod
    def filter_documents_by_date(documents: List[Dict[str, Any]], target_date: str) -> List[Dict[str, Any]]:
        if not target_date or target_date == "All Dates": return documents
        filtered_docs = []
        for doc in documents:
            if doc_date := doc.get("date"):
                try: parsed_doc_date = str(pd.to_datetime(doc_date).date())
                except Exception: parsed_doc_date = doc_date
    ...
    # --- NUEVA FUNCIÓN ---
    @staticmethod
    def extract_and_count_tags(documents: List[Dict[str, Any]], tag_key: str) -> pd.DataFrame:
    ...
    @staticmethod
    def filter_documents_by_tag(documents: List[Dict[str, Any]], selected_tag: str, tag_key: str) -> List[Dict[str, Any]]:
...
class EmisDocumentApp:
    ...
    def _get_emis_client(_self, api_token: str):
        ...
        try: return EmisDocuments(api_key=api_token)
        except Exception as e:
    ...
    def _render_login_page(self) -> None:
        ...
        with col2:
            st.image("static/emis-logo.png", width=800)
        
        col1, col2, col3 = st.columns([1.5, 2, 1])
        with col2:
            st.title("Login to EMIS Analyzer")
            st.caption("Please enter your EMIS API Token to continue.")

        col1, col2, col3 = st.columns([1, 1.5, 1])

        with col2:
            with st.form("login_form"):
                emis_token = st.text_input("EMIS API Token", type="password", label_visibility="collapsed", placeholder="EMIS API Token")
    ...
    def _render_main_app(self) -> None:
        ...
        if not self._ensure_client_initialized(): return
        ...
        if self.emis_client is None:
            ...
            if st.session_state.logged_in and current_token:
                try:
                    ...
                    st.error(f"Client re-initialization failed: {e}. Please log out and in again.")
    ...
    def _handle_search(self, search_params: SearchParameters) -> None:
        if not any([search_params.country_name, search_params.company_name, search_params.industry_name, search_params.keyword]):
            st.warning("Please provide at least one search criterion.")
            return
        
        self.session_manager.reset_search_data()
        parts = [f"about '{p}'" for p in [search_params.company_name, search_params.country_name, search_params.industry_name, search_params.keyword] if p]
    ...
    def _store_search_results(self, result: Dict[str, Any], search_params: SearchParameters) -> None:
        ...
        st.session_state.search_info = {**vars(search_params), **{k: result.get(k) for k in ["country_code", "company_id", "matched_company_name", "industry_code"]}}
    
    # --- MÉTODO MODIFICADO ---
    def _render_results(self) -> None:
        """Render search results, including analysis and filtered document list."""
        if st.session_state.result is None: return
        ...
        with st.container(border=True):
            ...
            st.markdown("---")
            self.ui_components.render_topics_wordcloud(documents)
    ...
    def _render_individual_documents_section(self, documents: List[Dict[str, Any]]) -> None:
        ...
        st.info(f"Showing {len(filtered_docs)} of {len(documents)} documents based on active filters.")
        self._render_document_list(filtered_docs)

    def _apply_document_filters(self, documents: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        industry_df = self.document_filter.extract_and_count_tags(documents, "industries")
        ...
        with col1:
            if not topic_df.empty:
                ...
                filtered_docs = self.document_filter.filter_documents_by_tag(filtered_docs, selected, "topics")
        with col2:
            if not industry_df.empty:
                options = ["All Industries"] + industry_df['name'].tolist()
                selected = st.selectbox("Filter by Industry:", options=options)
                filtered_docs = self.document_filter.filter_documents_by_tag(filtered_docs, selected, "industries")
        with col3:
            ...
            if unique_dates:
                ...
                filtered_docs = self.document_filter.filter_documents_by_date(filtered_docs, selected)
    ...
    def _render_document_list(self, documents: List[Dict[str, Any]]) -> None:
        ...
        for idx, doc in enumerate(documents):
            ...
            if idx < len(documents) - 1:
                st.markdown("""<hr style="height: 2px; background-color: #e0e0e0;">""", unsafe_allow_html=True)
    
    def _render_single_document(self, doc: Dict[str, Any], idx: int) -> None:
        doc_id, title, date, abstract = doc.get("id"), doc.get("title"), doc.get("date"), doc.get("abstract", "")
        with st.container(border=True):
