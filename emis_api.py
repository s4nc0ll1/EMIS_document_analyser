...
class SearchValidator:
    ...
    def validate_search_parameters(params: SearchParameters) -> None:
        ...
        if params.limit > 5000: logger.warning(f"Large limit specified: {params.limit}")
...
class EmisDocuments:
    ...
    def _search_documents(self, resolved_params: Dict[str, Any]) -> List[Document]:
        ...
        total_limit = api_call_params.pop("limit", 100) # El límite total deseado
        DOCUMENTS_PER_PAGE = 100 # Máximo por llamada a la API
        all_documents = []
        
        logger.info(f"Starting paginated document search up to {total_limit} documents.")

        for page in range((total_limit + DOCUMENTS_PER_PAGE - 1) // DOCUMENTS_PER_PAGE):
            offset = page * DOCUMENTS_PER_PAGE
            
            page_params = api_call_params.copy()
            page_params['limit'] = DOCUMENTS_PER_PAGE
            page_params['offset'] = offset
            
            if len(all_documents) >= total_limit:
                break

            logger.info(f"Searching documents with parameters: {page_params}")
            try:
                response = self.http_client.get(APIEndpoint.DOCUMENTS_SEARCH.value, params=page_params)
                documents_page = self.document_processor.process_documents(response)
                
                if not documents_page:
                    logger.info(f"No more documents found on page {page + 1}. Stopping search.")
                    break # Si una página viene vacía, no hay más resultados
                    
                all_documents.extend(documents_page)
                logger.info(f"Retrieved {len(documents_page)} documents from page {page + 1}. Total: {len(all_documents)}")
                
            except APIClientError as e:
                logger.error(f"Document search failed on page {page + 1}: {e}")
                break # Detener en caso de error

        final_documents = all_documents[:total_limit]
        logger.info(f"Successfully finished search. Total documents retrieved: {len(final_documents)}")
